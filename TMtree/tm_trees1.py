    ...
    *exactly* the same, so this doctest _might_ not pass when run on
    ...
    for item in ordered_listdir(path):
        if os.path.isdir(os.path.join(path, item)):
            ...
            content_list.append(
                (item, os.path.getsize(os.path.join(path, item)) + 1))
...
def moves_to_nested_dict(moves: list[list[str]]) -> dict[tuple[str, int], dict]:
    ...
    for key in group_by_first:
        res[(key, end_time_map[key])] = moves_to_nested_dict(
            group_by_first[key])
    return res
...
class TMTree:
    ...
    def is_displayed_tree_leaf(self) -> bool:
        ...
        False
        """
        if (self._parent_tree is None or self._parent_tree._expanded is True) \
    ...
    def get_path_string(self) -> str:
        ...
        while curr is not None:
            temp.append(curr._name)
            curr = curr._parent_tree
        temp.reverse()
        return self.get_separator().join(temp) + self.get_suffix()
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if rect[2] > rect[3]:
            ...
            flag = 'y'
            start = rect[1]
        all_data_size = sum([subtree.data_size for subtree in self._subtrees])
        for _subtree in self._subtrees:
            sub_rect = list(rect)
            if flag == 'x':
                sub_rect[0] = start
                start += int(_subtree.data_size * rect[2] / all_data_size)
                sub_rect[2] = int(_subtree.data_size * rect[2] / all_data_size)
                _subtree.update_rectangles((sub_rect[0], sub_rect[1],
                                            sub_rect[2], sub_rect[3]))
            elif flag == 'y':
                sub_rect[1] = start
                start += int(_subtree.data_size * rect[3] / all_data_size)
                sub_rect[3] = int(_subtree.data_size * rect[3] / all_data_size)
                _subtree.update_rectangles((sub_rect[0], sub_rect[1],
                                            sub_rect[2], sub_rect[3]))
        if self._subtrees:
            if flag == 'x':
                last_rect = (self._subtrees[-1].rect[0],
                             self._subtrees[-1].rect[1],
                             self._subtrees[-1].rect[2] + rect[2]
                             - sum([subtree.rect[2] for subtree in
                                    self._subtrees]),
                             self._subtrees[-1].rect[3])
                self._subtrees[-1].update_rectangles(last_rect)
            elif flag == 'y':
                last_rect = (self._subtrees[-1].rect[0],
                             self._subtrees[-1].rect[1],
                             self._subtrees[-1].rect[2],
                             self._subtrees[-1].rect[3]
                             + rect[3] - sum([subtree.rect[3]
                                              for subtree in self._subtrees]))
                self._subtrees[-1].update_rectangles(last_rect)

    def _find_leaf(self) -> list[TMTree]:
        res = []
        if self.is_displayed_tree_leaf():
            return [self]
        else:
            for subtree in self._subtrees:
                res.extend(subtree._find_leaf())
        return res
    ...
    def get_rectangles(self) -> list[tuple[tuple[int, int, int, int],
        ...
        if self.is_displayed_tree_leaf():
            ...
            leafs = []
            for subtree in self._subtrees:
                leafs.extend(subtree._find_leaf())
    ...
    def get_tree_at_position(self, pos: tuple[int, int]) -> Optional[TMTree]:
        ...
        if self.is_displayed_tree_leaf():
            ...
            for subtree in self._subtrees:
                leafs.extend(subtree._find_leaf())
        rect_list = [i.rect for i in leafs]
        for rect, leaf in zip(rect_list, leafs):
            if rect[0] <= pos[0] <= rect[0] + rect[2] and rect[1] <= pos[1] <= \
                    rect[1] + rect[3]:
                return leaf
        return None
    ...
    def change_size(self, factor: float) -> None:
        ...
        self.data_size = max(self.data_size, sum([i.data_size
                                                  for i in self._subtrees]))
        n = self.data_size - old_data_size
        if self._parent_tree:
            self._parent_tree.__check_data_size(n)

    def __check_data_size(self, n: int) -> None:
        self.data_size = max(self.data_size + n, sum([i.data_size
                                                      for i in self._subtrees]))
        if self._parent_tree:
            self._parent_tree.__check_data_size(n)
        else:
...
class ChessTree(TMTree):
    ...
    def __init__(self, move_dict: dict[tuple[str, int], dict],
        ...
        for key in move_dict:
            subtrees.append(ChessTree(move_dict=move_dict[key],
                                      last_move=key[0],
                                      white_to_play=not white_to_play,
                                      num_games_ended=key[1]))

        super().__init__(name=last_move, subtrees=subtrees,
                         data_size=num_games_ended)
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if rect[2] > rect[3]:
            ...
            start = rect[1]
            rest = rect[3]
        all_data_size = sum([iter_subtree.data_size for iter_subtree
                             in self._subtrees])
        ...
        for subtree in self._subtrees:
            ...
            if flag == 'x':
                ...
                if last_idx == now_idx:
                    subtree.update_rectangles((sub_rect[0], sub_rect[1],
                                               rest, sub_rect[3]))
                else:
                    subtree.update_rectangles((sub_rect[0], sub_rect[1],
                                               sub_rect[2], sub_rect[3]))
