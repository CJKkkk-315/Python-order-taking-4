...
def run_genetic_algorithm(coords, population_size=200, generations=500, crossover_prob=0.1, mutation_prob=0.1, elite_size=0.1):
    ...
    elite_num = max(1, int(population_size * elite_size))

    for _ in range(generations):
        population.sort(key=lambda individual: individual.distance)
        elites = population[:elite_num]  # select elite individuals
        ...
        population = elites + new_population[elite_num:]
    return population[0]
...
best_individual = run_genetic_algorithm(coords, elite_size=0)
end_time = time.time()
