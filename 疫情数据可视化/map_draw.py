...
files = os.listdir('2022-12-01')
json_data = []
for file in files:
    json_data.append([file.split('_')[0].replace('市','').replace('省','')
                         .replace('维吾尔自治区','')
                         .replace('壮族自治区','')
                         .replace('回族自治区','')
                         .replace('自治区',''),json.load(open('2022-12-01/' + file,encoding='utf8'))[-1]['confirmedCount']])

print(json_data)
max_data = max([i[1] for i in json_data])
(
    Map()
    .add(
        series_name="累计确诊",
    ...
    # 设置分段显示
    visualmap_opts=opts.VisualMapOpts(max_=10000,range_color=['#e6f7ff', '#1890FF','#0050b3'])
    )
    # 生成本地html文件
    .render("省份确诊人数.html")
)
    ...
    for k, v in area_data.items():
        for j in v:
            if i[6] in j:
                if k not in pro_emo:
                    pro_emo[k] = []
                pro_emo[k].append(snownlp.SnowNLP(i[4]).sentiments)
xy = []
for k,v in pro_emo.items():
...
(
    ...
    .set_global_opts(
    title_opts=opts.TitleOpts(title="中国地图"),
    visualmap_opts=opts.VisualMapOpts(max_=1,is_piecewise=True,range_color=['#e6f7ff', '#1890FF','#0050b3'])
    )
    .render("省份情感得分.html")
)
...
data = []
with open('热评.csv',encoding='utf8') as f:
    f_csv = csv.reader(f)
    for i in f_csv:
        data.append(i)
data = [i for i in data[1:] if i[6]]
...
(
    ...
    .set_global_opts(
    title_opts=opts.TitleOpts(title="中国地图"),
    visualmap_opts=opts.VisualMapOpts(max_=10,range_color=['#e6f7ff', '#1890FF','#0050b3'])
    )
    .render("省份热评数量.html")
)
