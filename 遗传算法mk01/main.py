...
for e in range(10):
    ...
    res_list.append([result,job,machine,machine_time])
    print(f'第{e+1}次算法进行结果：{result[:,1][-1]}')
res_list.sort(key=lambda x:x[0][:,1][-1])
final_result,final_job,final_machine,final_machine_time = res_list[0]
...
plt.ylabel("时间")
plt.show()
to.draw(final_job,final_machine,final_machine_time)