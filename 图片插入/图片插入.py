    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            if file.endswith(".jpg") or file.endswith(".jpeg"):
                ...
                if file_name in df.iloc[:, 1].values:
                    row_index = df.index[df.iloc[:, 1] == file_name][0] + 2  # pandas 从0开始，openpyxl 从1开始，且排除了标题行
                    img = Image(os.path.join(root, file))
                    img.width = 60
                    img.height = 80
                    ws.add_image(img, f'F{row_index}')  # 'F'表示第六列

    # 保存工作簿
    wb.save("new_excel.xlsx")

# 调用函数
insert_images_to_excel('定价体系测试.xlsx', '图片2')
