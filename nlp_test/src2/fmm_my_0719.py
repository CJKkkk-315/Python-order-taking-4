...
def read_similar_words():  #输入一个词语，返回的是该词的近义词的集合
    global similar_words_dict, similar_words_list
...
def init():
    global inv_index, name_list, doc_list, loc_set, dict_loc, file_dict_lines, N
    # name = 'merchant_life3'
...
def query(seg_q, num, inv_index, db_list, id_part):
    ...
    for sub in seg_q:  #每个查询词的分词
        ...
        if (name_list[id_part] == 'merchant_bill3') and (sub in dict_loc or (sub + '市') in dict_loc):  #搜索生活缴费时，需把地区从分词列表中滤掉。
            continue;
        if sub in inv_index:  #在倒索引里
            L1_ID = inv_index[sub]  #某个分词命中的文档列表
            for id in L1_ID:  #id是文档也就是标题的id
                if (name_list[id_part] == 'merchant_bill3' and id in loc_sel_set) or (name_list[id_part] != 'merchant_bill3'):  #id_part == 3表示属于缴费类型，做特殊处理
                    if id in q_score:
                        if id_sub<=num:
...
for id_part in range(N):  #对N类文档循环。分好词后，匹配几乎不耗时间
    ...
    print(f'***{id_part}、{name_ch}模块有：{len(r)}个：')
    num = 1
    for s in r:
        id = int(s[0])
        score = s[1]
        key_word = w[s[0]]
        print(num, doc_list[id_part][id], key_word)
        if id<=len(doc_list[id_part])-1:
            print(num, doc_list[id_part][id], key_word)   #输出序号、文档内容、命中的关键词
        num+=1;
