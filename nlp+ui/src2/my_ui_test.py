...
class FMMSegment(BaseSegment):
    ...
    def cut(self, sentence: str):
        ...
        while text_size > index:
            ...
            index = index + 1
            yield word
def allf(text, inv_index, name_list, doc_list, loc_set, dict_loc, file_dict_lines, N, my_ins):
    ...
    def init():
        ...
        print(1)
        return FMMSegment(file_dict_lines)

    def app_segment(q, my_ins):  # 分词函数，输入为q
        # path_dict = f'../data/dict/dict_all_0716.txt'
        stop_words = ('的', '我', '在', '怎么', '要')  # 停用词
        num = 0  # 除去近义词以外的关键词个数
        sub_query = list((my_ins.cut(q)))  # 分词结果
    ...
    if inv_index is None:
        ...
        dict_loc = {}
        file_dict_lines = 0
        my_ins = init()  # 初始化，加载各种文件
    else:
        read_similar_words()

    start_time = time.time()  # 程序开始时间

    for i in range(100):  # 循环十遍，计算平均时间
        res = app_segment(q, my_ins)  # 分词的结果和个数
        seg_q = res[0]
        num = res[1]
    ...
    return uires, round(run_time*10,4), inv_index, name_list, doc_list, loc_set, dict_loc, file_dict_lines, N, my_ins
